...
BEGIN
    ...
    WHERE
            tf.Date_Realisation
            -- pour tester à remplacer 'sysdate' avec 'TO_DATE('24-03-2024', 'DD-MM-YYYY')'
            BETWEEN NEXT_DAY(TRUNC(TO_DATE('24-03-2024', 'DD-MM-YYYY') - 7), 'MONDAY')
            -- idem, sinon il faut créer des nouvelles tâches chaque semaine...
            AND TRUNC(TO_DATE('24-03-2024', 'DD-MM-YYYY'))
...
// Procédure pour afficher les données du curseur:
SET SERVEROUTPUT ON;
DECLARE
    c_result_cursor SYS_REFCURSOR;
    c_ref_utilisateur Utilisateur.ref_utilisateur%TYPE;
    c_login_utilisateur Utilisateur.login%TYPE;
    c_points_gagnes NUMBER;

BEGIN
    c_result_cursor := calculer_points_semaine();

    LOOP
        FETCH c_result_cursor 
        INTO c_ref_utilisateur, 
             c_login_utilisateur, 
             c_points_gagnes;
        EXIT WHEN c_result_cursor%NOTFOUND;

        DBMS_OUTPUT.PUT_LINE('User_ID : ' || c_ref_utilisateur || ', User_Login : ' 
        || c_login_utilisateur || ', Points Gagnes : ' || c_points_gagnes);
    END LOOP;
...
BEGIN
    ...
    WHERE
        ...
        AND tec.date_realisation -- TO_DATE('19-02-2024', 'DD-MM-YYYY')
            BETWEEN NEXT_DAY(TRUNC(TO_DATE('19-02-2024', 'DD-MM-YYYY') - 8), 'MONDAY')
            AND TRUNC(TO_DATE('19-02-2024', 'DD-MM-YYYY') - 1);
    ...
    FROM
        Tache_en_cours tec
    WHERE
        tec.date_d_echeance <= TRUNC(TO_DATE('19-02-2024', 'DD-MM-YYYY') - 1) + INTERVAL '23:59:59' HOUR TO SECOND
    ...
    WHERE
        ...
        BETWEEN NEXT_DAY(TRUNC(TO_DATE('19-02-2024', 'DD-MM-YYYY') - 8), 'MONDAY') AND TRUNC(TO_DATE('19-02-2024', 'DD-MM-YYYY') - 1)) -- TO_DATE('19-02-2024', 'DD-MM-YYYY')
        OR (date_d_echeance <= TRUNC(TO_DATE('19-02-2024', 'DD-MM-YYYY') - 1) + INTERVAL '23:59:59' HOUR TO SECOND AND date_realisation IS NULL));
END;
/

EXECUTE taches_en_cours_devient_tache_fini();
...
EXECUTE generate_task_suggestions(0, 5, 4, 2);
